...
public class HomeWorkApp {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            g.playerTurn();
            g.printMap();
            if (g.checkWin(g.player)) { System.out.println("Поздравляю победа"); break; }
            if (g.isMapFull()) { System.out.println("Game over. Никто не победил"); break; }

            //AI-1 turn
            g.aiTurn(g.cpu1);
            g.printMap();
            if (g.checkWin(g.cpu1)) { System.out.println("Game over. Компьютер Победил"); break; }
            if (g.isMapFull()) { System.out.println("Game over. Никто не победил"); break; }
